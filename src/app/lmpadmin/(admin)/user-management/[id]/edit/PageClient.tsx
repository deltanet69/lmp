...
export default function EditUserPage() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            const { data, error } = await supabase.from("useradmin").select("*").eq("id", id).single();
            if (data) {
                setFormData({
    ...
    if (fetching) return <div className="p-8 text-slate-400">Loading user data...</div>;
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Update account details for {formData.full_name}</p>
                    </div>
                </div>
            </div>
